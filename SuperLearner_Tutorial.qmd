...
The optimal prediction rule, with respect to the specified loss function, is
defined as the rule that minimizes the expected risk,
...
For each fold $v = 1, \ldots, V$, let $V(v)$ denote the validation set and
$T(v)$ its complementary training set. Each candidate algorithm $\mathcal A_k$
is fitted on $T(v)$ and generates predictions for the observations in
$V(v)$. Because these predictions are obtained from models that were not
fitted using the corresponding validation observations, they constitute
cross-validated, or out-of-fold, predictions.

The predictions obtained for the $K$ candidate algorithms are collected in an
$n \times K$ matrix $Z$,
...
Let $\alpha=(\alpha_1,\ldots,\alpha_K)$ denote the vector of weights assigned
to the candidate algorithms. The meta-learner selects the weights that minimize
...
$$
where $R_{CV}(\alpha)$ denotes the cross-validated risk of the weighted
combination under the specified performance criterion. The specific form of
this criterion depends on the meta-learning procedure used to estimate the
ensemble weights.
...
Once the weights have been estimated, each candidate algorithm is refitted
using the full sample. For a new set of covariate values $w$, the final Super
Learner prediction is
...
The main theoretical guarantee of Super Learner is based on an oracle-type
result. In this setting, the oracle corresponds to the combination that
achieves the lowest risk within the considered family of combinations. As
described above, Super Learner uses cross-validation to learn the combination
of the candidate algorithms; the oracle-type result establishes that its
performance asymptotically approaches that of this reference.
